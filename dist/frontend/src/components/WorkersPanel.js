...
function WorkersPanel({ onClose }) {
    const { kingdom, buildings, refresh } = (0, gameStore_1.useGameStore)();
    const maxWorkers = (0, gameStore_1.useGameStore)(s => s.kingdom?.maxWorkers ?? 5);
    ...
    return (<div style={{ position: 'fixed', inset: 0, zIndex: 600, background: 'rgba(0,0,0,0.85)' }} onClick={e => { if (e.target === e.currentTarget)
        onClose(); }}>
      <div style={{
        ...
        <div style={{ background: 'rgba(39,174,96,0.1)', border: '1px solid rgba(39,174,96,0.25)', borderRadius: 14, padding: '16px', marginBottom: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <div style={{ textAlign: 'right', fontSize: 12, color: '#a0845a' }}>
              ...
              <div>{t('salary')}: <span style={{ color: '#e74c3c', fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: 3 }}>-{(0, format_1.fmt)(workerSalary)} <ResIcon_1.ResIcon type="gold"/>{t('per_hour')}</span></div>
              {workerFoodPerHour > 0 && <div>אוכל: <span style={{ color: '#e74c3c', fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: 3 }}>-{workerFoodPerHour} <ResIcon_1.ResIcon type="food"/>{t('per_hour')}</span></div>}
            </div>
          </div>
        ...
        {hasAcademy && (<>
          <div style={{ background: 'rgba(144,80,200,0.08)', border: '1px solid rgba(144,80,200,0.25)', borderRadius: 14, padding: '14px 16px', marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              ...
              {explorerCount > 0 && (<div style={{ textAlign: 'right', fontSize: 12, color: '#a0845a' }}>
                  <div>{t('salary')}: <span style={{ color: '#e74c3c', fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: 3 }}>-{explorerGoldPerHour} <ResIcon_1.ResIcon type="gold"/>{t('per_hour')}</span></div>
                  <div>{t('food')}: <span style={{ color: '#e74c3c', fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: 3 }}>-{explorerFoodPerHour} <ResIcon_1.ResIcon type="food"/>{t('per_hour')}</span></div>
                </div>)}
            </div>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {explorerCount === 0
                ? <div style={{ fontSize: 12, color: '#666' }}>{t('exp_need_explorer')}</div>
                : Array.from({ length: explorerCount }).map((_, i) => (<div key={i} style={{ width: 36, height: 36, borderRadius: 8, background: 'rgba(144,80,200,0.2)', border: '1px solid rgba(144,80,200,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>
                      🧭
                    </div>))}
            </div>
          </div>
