...
function preload(srcs, cb) {
    ...
    keys.forEach(k => {
        ...
        const img = new Image();
        img.onload = img.onerror = () => { loadedImgs[k] = img; if (--n === 0)
            cb(); };
        img.src = srcs[k];
    });
}
function kingdomKey(lvl) {
...
function WorldMapScreen() {
    ...
    const { buildings, refresh, units, kingdom } = (0, gameStore_1.useGameStore)();
    const canvasRef = (0, react_1.useRef)(null);
    const containerRef = (0, react_1.useRef)(null);
    const [mapData, setMapData] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [imgsReady, setImgsReady] = (0, react_1.useState)(false);
    ...
    const [fogMsg, setFogMsg] = (0, react_1.useState)('');
    ...
    const centered = (0, react_1.useRef)(false);
    (0, react_1.useEffect)(() => {
        if (loading || centered.current || !containerRef.current)
            return;
    ...
    (0, react_1.useEffect)(() => {
        ...
        mapData?.nodes.filter(n => n.discovered).forEach(n => {
            ...
            if (img) {
                ...
                ctx.save();
                ctx.beginPath();
                ctx.arc(sx, sy, r - zoom * 0.5, 0, Math.PI * 2);
                ctx.clip();
        ...
        if (sendTarget) {
            ...
            ctx.beginPath();
            ctx.arc(sx, sy, 16 * zoom, 0, Math.PI * 2);
            ctx.strokeStyle = '#f4d03f';
    ...
    const pinchRef = (0, react_1.useRef)(null);
    ...
    if (loading)
        return (<div style={{ textAlign: 'center', paddingTop: 80, color: '#a0845a' }}>{t('loading_map')}</div>);
    return (<div className="screen" style={{ padding: 0, display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {actionMsg && (<div style={{ padding: '6px 14px', fontSize: 12, textAlign: 'center', flexShrink: 0,
      ...
      <div ref={containerRef} style={{ position: 'absolute', inset: 0, background: '#0a1a08', touchAction: 'none', userSelect: 'none', visibility: tab === 'map' ? 'visible' : 'hidden', pointerEvents: tab === 'map' ? 'auto' : 'none' }}>
          <canvas ref={canvasRef} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} onWheel={onWheel} style={{ display: 'block', cursor: 'grab' }}/>

          

          
          <div style={{ position: 'absolute', top: 8, right: 8, display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'center' }}>
            {[['＋', 1.2], ['－', 0.83]].map(([label, f]) => (<button key={label} onClick={() => setZoom(z => Math.max(0.3, Math.min(3, z * f)))} style={{ width: 30, height: 30, borderRadius: 6, background: 'rgba(0,0,0,0.75)', border: '1px solid rgba(255,255,255,0.15)', color: '#fff', fontSize: 16, cursor: 'pointer', lineHeight: 1 }}>
                {label}
              </button>))}
          </div>

          
          {!selected && !sendTarget && hasAcademy && freeExplorers > 0 && (<div style={{ position: 'absolute', top: 10, left: '50%', transform: 'translateX(-50%)', background: 'rgba(0,0,0,0.72)', border: '1px solid rgba(244,208,63,0.25)', borderRadius: 20, padding: '5px 14px', pointerEvents: 'none', whiteSpace: 'nowrap' }}>
              <span style={{ fontSize: 11, color: '#c8a84b' }}>👆 {t('exp_send_hint')}</span>
            </div>)}
          {!selected && !sendTarget && hasAcademy && freeExplorers === 0 && activeCount > 0 && (<div style={{ position: 'absolute', top: 10, left: '50%', transform: 'translateX(-50%)', background: 'rgba(0,0,0,0.72)', border: '1px solid rgba(244,208,63,0.25)', borderRadius: 20, padding: '5px 14px', pointerEvents: 'none', whiteSpace: 'nowrap' }}>
              <span style={{ fontSize: 11, color: '#c8a84b' }}>{t('exp_all_busy', { n: activeCount })}</span>
            </div>)}

          
          {fogMsg && (<div style={{ position: 'absolute', top: 14, left: '50%', transform: 'translateX(-50%)', background: 'rgba(0,0,0,0.9)', border: '1px solid rgba(244,208,63,0.4)', borderRadius: 14, padding: '8px 16px', whiteSpace: 'nowrap', pointerEvents: 'none', zIndex: 20 }}>
              <span style={{ fontSize: 12, color: '#f4d03f' }}>{fogMsg}</span>
            </div>)}

          
          {!hasAcademy && (<div style={{ position: 'absolute', bottom: 16, left: 14, right: 14, background: 'rgba(0,0,0,0.88)', border: '1px solid rgba(244,208,63,0.3)', borderRadius: 12, padding: 12, textAlign: 'center' }}>
          ...
          {sendTarget && (<div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(180deg,rgba(15,8,0,0.97),rgba(8,12,0,0.99))', borderTop: '1px solid rgba(244,208,63,0.3)', padding: 14 }}>
          ...
          {selected && !sendTarget && (<div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(180deg,rgba(15,8,0,0.97),rgba(8,12,0,0.99))', borderTop: '1px solid rgba(244,208,63,0.3)', padding: 14 }}>
              {selected.type === 'hero' ? (<HeroNodePanel heroType={selected.heroType} alreadyRecruited={(units ?? []).some(u => u.type === selected.heroType && u.count > 0)} onRecruit={recruitHero} t={t}/>) : (<>
                  <div style={{ fontSize: 14, fontWeight: 900, color: selected.type === 'rare_resource' ? '#16a085' : '#b8860b', marginBottom: 4 }}>
                    {selected.resourceType ? <ResIcon_1.ResIcon type={selected.resourceType} size={16}/> : '📦'} {t(selected.resourceType)} · ~{selected.amount?.toLocaleString()}
                    {selected.type === 'rare_resource' && <span style={{ fontSize: 11, fontWeight: 400, color: '#16a085' }}> {t('exp_rare')}</span>}
                  </div>
                  {selected.lastRaidedAt && !selected.canRaid && (<div style={{ fontSize: 11, color: '#a0845a', marginBottom: 10 }}>
                      ⏳ <Countdown_1.default endsAt={new Date(new Date(selected.lastRaidedAt).getTime() + 86400000).toISOString()} onEnd={load}/>
                    </div>)}
                  <button onClick={selected.canRaid ? raidNode : undefined} disabled={!selected.canRaid} style={{ width: '100%', padding: 10, borderRadius: 10, background: selected.canRaid ? 'linear-gradient(135deg,#f39c12,#f4d03f)' : 'rgba(255,255,255,0.08)', border: 'none', color: selected.canRaid ? '#000' : '#555', fontWeight: 900, fontSize: 14, cursor: selected.canRaid ? 'pointer' : 'not-allowed' }}>
                    {selected.canRaid ? <><ResIcon_1.ResIcon type={selected.resourceType ?? ''} size={14}/> {t('node_attack_btn')}</> : t('node_cooldown_active')}
                  </button>
                </>)}
      ...
      {tab === 'missions' && (<div style={{ position: 'absolute', inset: 0, overflowY: 'auto', padding: 14, background: 'linear-gradient(180deg,#0f0800,#080c00)', zIndex: 1 }}>
          <div style={{ background: 'linear-gradient(135deg,rgba(39,174,96,0.1),rgba(26,138,64,0.05))', border: '1px solid rgba(39,174,96,0.25)', borderRadius: 14, padding: 14, marginBottom: 12 }}>
            ...
            {(mapData?.trainingQueue ?? []).length > 0 && (<div style={{ background: 'rgba(52,152,219,0.1)', border: '1px solid rgba(52,152,219,0.3)', borderRadius: 10, padding: '8px 12px', marginBottom: 8 }}>
                <div style={{ fontSize: 12, color: '#3498db', fontWeight: 700, marginBottom: 4 }}>{t('exp_training')}</div>
                {[...mapData.trainingQueue].sort().map((endsAt, i) => (<div key={endsAt} style={{ fontSize: 11, color: '#a0845a', marginTop: 3, display: 'flex', justifyContent: 'space-between' }}>
                    <span>🧭 {t('explorer')} #{explorerCount + i + 1}</span>
                    <Countdown_1.default endsAt={endsAt} onEnd={load}/>
                  </div>))}
              </div>)}
            ...
            {totalExplorers >= maxExp && maxExp > 0 && (<div style={{ fontSize: 11, color: '#27ae60', textAlign: 'center' }}>
          ...
          {activeCount > 0 && <>
            ...
            {mapData.activeMissions.map(m => (<div key={m.id} style={{ background: 'rgba(244,208,63,0.07)', border: '1px solid rgba(244,208,63,0.2)', borderRadius: 12, padding: '10px 12px', marginBottom: 8 }}>
                ...
                <div style={{ fontSize: 11, color: '#a0845a', marginTop: 2 }}>{t('exp_returns')} <Countdown_1.default endsAt={m.returnsAt} onEnd={load}/></div>
              </div>))}
          </>}
          ...
          {(mapData?.returnedMissions.length ?? 0) > 0 && <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, marginBottom: 8 }}>
              ...
              <button onClick={async () => { await client_1.api.delete('/exploration/missions/completed'); await load(); }} style={{ fontSize: 11, padding: '3px 10px', borderRadius: 8, background: 'rgba(231,76,60,0.15)', border: '1px solid rgba(231,76,60,0.3)', color: '#e74c3c', cursor: 'pointer' }}>
                🗑️ {t('clear')}
              </button>
            </div>
            {mapData.returnedMissions.map(m => {
                    const nodeMap = new Map(mapData.nodes.map(n => [n.id, n]));
                    const found = (m.discoveredNodeIds ?? []).map(id => nodeMap.get(id)).filter(Boolean);
                    const nodeLabel = (n) => {
                        if (n.type === 'hero')
                            return <>{t('exp_hero_node')} {t('u_' + n.heroType)}</>;
                        const resType = n.resourceType ?? '';
                        const rarity = n.type === 'rare_resource' ? ` ${t('exp_rare')}` : '';
                        return <><ResIcon_1.ResIcon type={resType} size={11}/> {t(n.resourceType)}{rarity}</>;
                    };
                    return (<div key={m.id} style={{ background: 'rgba(39,174,96,0.07)', border: '1px solid rgba(39,174,96,0.15)', borderRadius: 12, padding: '10px 12px', marginBottom: 8 }}>
          ...
          {explorerCount === 0 && academyLevel === 0 && (<div style={{ textAlign: 'center', color: '#666', paddingTop: 40, fontSize: 12 }}>{t('exp_no_academy')}</div>)}
        </div>)}
      </div>
    </div>);
}
