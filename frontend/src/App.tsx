...
// Global error boundary — instead of a white screen on any render crash,
// show a friendly Hebrew message with a reload button.
function ErrorFallback() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', padding: 24, textAlign: 'center', gap: 16 }}>
      ...
      <button
        style={{ padding: '12px 28px', background: 'linear-gradient(135deg,#f4d03f,#b8860b)', borderRadius: 10, color: '#1a0a00', fontWeight: 700, border: 'none', cursor: 'pointer', fontSize: 16 }}
...
const INTERSTITIAL_INTERVAL_MS = 5 * 60 * 1000; // at most once every 5 minutes

function showInterstitial(isVip = false) {
  if (isVip) return; // VIP users are ad-free
  const AdController = (window as any).Adsgram?.init({ blockId: INTERSTITIAL_BLOCK_ID });
  AdController?.show().catch(() => {});
}

function BattleErrorPopup({ message, onClose }: { message: string; onClose: () => void }) {
  const t = useT();
  const { setScreen } = useGameStore();
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 8500, background: 'rgba(0,0,0,0.92)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: 24 }}>
...
function BattleResultPopup({ report, onClose, onAttackAgain }: { report: any; onClose: () => void; onAttackAgain: () => void }) {
  const t = useT();
  const { setScreen, buildings, units } = useGameStore();
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 8000, background: 'rgba(0,0,0,0.92)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: 24, overflowY: 'auto' }}>
      ...
      {report.attackerWins && (report.winStreak ?? 0) >= 2 && (
        <div style={{ background: 'rgba(244,208,63,0.15)', border: '1px solid #f4d03f', borderRadius: 20, padding: '5px 16px', color: '#f4d03f', fontWeight: 700, fontSize: 14 }}>
          {t('win_streak', { n: report.winStreak ?? 0 })}
          {(report.streakBonus ?? 0) > 0 && <span> · +{fmt(report.streakBonus)} <img src="/assets/icon_gold.png" style={{width:14,height:14,verticalAlign:'middle'}} /></span>}
        </div>
      )}
      ...
      {report.attackerWins && (
        <div style={{ background: 'rgba(244,208,63,0.1)', border: '1px solid rgba(244,208,63,0.3)', borderRadius: 12, padding: '14px 28px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 16, fontSize: 16, fontWeight: 700, flexWrap: 'wrap', justifyContent: 'center' }}>
            <span><img src="/assets/icon_gold.png" style={{width:16,height:16,verticalAlign:'middle',marginRight:3}} />{fmt(report.loot.gold)}</span>
            <span><img src="/assets/icon_wood.png" style={{width:16,height:16,verticalAlign:'middle',marginRight:3}} />{fmt(report.loot.wood)}</span>
            <span><img src="/assets/icon_stone.png" style={{width:16,height:16,verticalAlign:'middle',marginRight:3}} />{fmt(report.loot.stone)}</span>
            {(report.loot.gems ?? 0) > 0 && <span style={{ color: '#a29bfe' }}><img src="/assets/icon_gem.png" style={{width:16,height:16,verticalAlign:'middle',marginRight:3}} />{fmt(report.loot.gems)}</span>}
      ...
      {report.attackerLosses && Object.values(report.attackerLosses).some((v: any) => v > 0) && (
        <div style={{ background: 'rgba(231,76,60,0.08)', border: '1px solid rgba(231,76,60,0.25)', borderRadius: 12, padding: '10px 18px', textAlign: 'center', width: '100%', maxWidth: 360 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 12, width: '100%' }}>
            {Object.entries(report.attackerLosses).filter(([, v]: any) => v > 0).map(([type, v]: any) => {
              const wounded = report.attackerWounded?.[type] ?? 0;
              const dead = v - wounded;
              return (
                <div key={type} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: '4px 10px' }}>
                  <span style={{ color: '#ccc', fontWeight: 600 }}>{t(('u_' + type) as any)}</span>
                  <span style={{ display: 'flex', gap: 10 }}>
                    {dead > 0 && <span style={{ color: '#e74c3c' }}>💀 {fmt(dead)}</span>}
                    {wounded > 0 && <span style={{ color: '#f39c12' }}>🏥 {fmt(wounded)}</span>}
                  </span>
                </div>
              );
            })}
          </div>
          {Object.values(report.attackerWounded ?? {}).some((v: any) => v > 0) && (() => {
      ...
      {report.defenderLosses && Object.values(report.defenderLosses).some((v: any) => v > 0) && (
        <div style={{ background: 'rgba(39,174,96,0.08)', border: '1px solid rgba(39,174,96,0.25)', borderRadius: 12, padding: '10px 18px', textAlign: 'center', width: '100%', maxWidth: 360 }}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', fontSize: 12 }}>
            {Object.entries(report.defenderLosses).filter(([, v]: any) => v > 0).map(([type, v]: any) => {
              const wounded = report.defenderWounded?.[type] ?? 0;
              return (
                <span key={type} style={{ color: '#27ae60' }}>
                  -{fmt(v - wounded)} {t(('u_' + type) as any)}
                  {wounded > 0 && <span style={{ color: '#f39c12' }}> 🏥{fmt(wounded)}</span>}
                </span>
              );
            })}
      ...
      {report.buildingDamaged && (
        <div style={{ background: 'rgba(230,126,34,0.12)', border: '1px solid rgba(230,126,34,0.4)', borderRadius: 12, padding: '12px 20px', textAlign: 'center', color: '#e67e22', fontWeight: 700, fontSize: 14 }}>
          💥 {t('building_damaged', { name: t(('b_' + report.buildingDamaged.type) as any), n: report.buildingDamaged.newLevel })}
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
        ...
        {report.attackerWins && (
          <button className="btn btn-green" style={{ padding: '12px 20px', fontSize: 14 }} onClick={() => {
            const tg = (window as any).Telegram?.WebApp;
            const loot = `💰${fmt(report.loot.gold)} 🪵${fmt(report.loot.wood)} 🪨${fmt(report.loot.stone)}`;
...
function DailyBonusPopup({ bonus, onClose }: { bonus: { gems: number; streak: number }; onClose: () => void }) {
  const t = useT();
...
function DevUserPicker({ onPick }: { onPick: (key: string) => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'linear-gradient(135deg,#0d0500,#1a0a00)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: 24, zIndex: 9999 }}>
      ...
      <button
        onClick={seed}
        disabled={seeding}
        style={{ padding: '8px 20px', background: '#2a1500', border: '1px solid #f4d03f', borderRadius: 8, color: '#f4d03f', cursor: 'pointer', fontSize: 13, marginBottom: 4 }}
      >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%', maxWidth: 300 }}>
        {DEV_USERS.map((u, i) => (
          <div key={u.key} style={{ display: 'flex', gap: 6 }}>
            <button
              onClick={() => { localStorage.setItem('kw_dev_user', u.key); onPick(u.key); }}
              style={{ flex: 1, padding: '12px 16px', background: `${u.color}22`, border: `2px solid ${u.color}`, borderRadius: 12, color: '#fff', cursor: 'pointer', fontSize: 15, fontWeight: 700, textAlign: 'left' }}
            >
              {u.name}
            </button>
            <button
              title="תן VIP + כל הגיבורים + הפניות"
              onClick={async () => {
...
function AppInner() {
  const { token, setToken, loadKingdom, activeScreen, kingdom, pendingBattleReport, clearPendingBattleReport, pendingError, clearPendingError } = useGameStore();
  const [authError, setAuthError] = useState('');
  const [dailyBonus, setDailyBonus] = useState<{ gems: number; streak: number } | null>(null);
  ...
  // Apply dir/lang on mount and language change
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (tg?.initData) {
      ...
      // Deep-link screen: ?screen=leaderboard (separate from referral params)
      ...
      // Read stored ref (if current URL has no ref, fall back to stored one)
      ...
      // If we have a saved token, load kingdom immediately (fast path)
      // But still run auth to check termsAccepted on every session
      if (savedToken) setToken(savedToken);

      api.post('/auth/login', { initData: tg.initData, referralCode: ref })
        .then(({ token, dailyBonus, termsAccepted, isNewUser }) => {
          setToken(token);
          // Clear the pending ref cookie after successful login (ref was applied)
          localStorage.removeItem(REF_KEY);
          localStorage.removeItem(REF_EXP);
          if (dailyBonus) setDailyBonus(dailyBonus);
          // Always show terms if not accepted — even for returning users
          if (!termsAccepted) {
            setNeedsTerms(true);
          } else if (isNewUser) {
  ...
  useEffect(() => {
    if (token) {
      loadKingdom();
      // Show interstitial after kingdom loads — read from store so VIP status is current
      setTimeout(() => {
        const { kingdom: k } = useGameStore.getState();
  ...
  // Show interstitial on screen transitions (throttled)
  useEffect(() => {
    if (!token || !kingdom) return;
    if (prevScreenRef.current === activeScreen) return;
    prevScreenRef.current = activeScreen;
    if (!INTERSTITIAL_SCREENS.has(activeScreen)) return;
    const now = Date.now();
    if (now - lastInterstitialRef.current < INTERSTITIAL_INTERVAL_MS) return;
  ...
  if (showDevPicker && import.meta.env.DEV) {
    return (
      <DevUserPicker onPick={(key) => {
        setShowDevPicker(false);
        api.post('/auth/login', { initData: key })
          .then(({ token }) => setToken(token))
  ...
  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {needsTerms && <TermsModal onAccepted={() => { setNeedsTerms(false); setShowOnboarding(true); }} />}
      {!needsTerms && showOnboarding && <OnboardingModal defaultName={tgFirstName} onDone={() => { setShowOnboarding(false); setShowTutorial(true); loadKingdom(); }} />}
      {!needsTerms && !showOnboarding && showTutorial && <TutorialOverlay onDone={() => setShowTutorial(false)} />}
      ...
      <ResourceBar />
      {/* Content fills space between ResourceBar and NavBar — NavBar is a normal flex item */}
      <div
        data-scroll-container
        key={activeScreen}
        style={{ flex: 1, minHeight: 0, position: 'relative', overflow: activeScreen === 'home' || activeScreen === 'worldmap' ? 'hidden' : 'auto', display: 'flex', flexDirection: 'column' }}>
        {activeScreen === 'home'        && <HomeScreen />}
        {activeScreen === 'repair'      && <RepairScreen />}
        ...
        {activeScreen === 'worldmap'    && <WorldMapScreen />}
