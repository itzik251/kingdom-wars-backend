...
const TABS = [
  ...
  { key: 'referral',   emoji: '👥', tkey: 'nav_referral'     },
  { key: 'contact',    emoji: '✉️', tkey: 'nav_contact'      },
] as const;
...
export default function NavBar() {
  ...
  const isVip = !!(kingdom as any)?.isVip;
    ...
    <>
      ...
      {open && (
        <div onClick={() => setOpen(false)}
          style={{ position: 'fixed', inset: 0, zIndex: 490, background: 'rgba(0,0,0,0.5)' }} />
      )}
      ...
      <div style={{
        ...
        {/* Kingdom name */}
        <button
          onClick={() => { window.dispatchEvent(new Event('open-rename')); setOpen(false); }}
          style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(244,208,63,0.07)', border: '1px solid rgba(244,208,63,0.15)', borderRadius: 10, padding: '7px 10px', cursor: 'pointer', marginBottom: 8 }}
        >
