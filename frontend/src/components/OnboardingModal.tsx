...
interface Props { defaultName: string; onDone: () => void; }

const KINGDOM_FLAGS = ['🏰','⚔️','🛡️','🦁','🐉','🌟','💎','🔥','❄️','🌙','⚡','🌊','🗡️','👑','🏯'];

const LABELS: Record<string, { title: string; sub: string; namePh: string; btn: string; tooShort: string; tooLong: string }> = {
...
export default function OnboardingModal({ defaultName, onDone }: Props) {
  ...
  async function submit() {
    ...
    if (trimmed.length < 3) { setError(lbl.tooShort); return; }
    if (trimmed.length > 20) { setError(lbl.tooLong); return; }
    setLoading(true);
    try {
