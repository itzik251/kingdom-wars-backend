...
export default function ResourceBar() {
  ...
  const workers = (kingdom as any).workers ?? 0;
  const explorers = (kingdom as any).explorerCount ?? 0;
  ...
  return (
    <div style={{
      ...
      {RESOURCES.map(({ key, icon, maxKey, color }) => {
        ...
        const max = maxKey ? (kingdom as any)[maxKey] ?? 0 : 0;
        ...
        return (
          <div key={key} title={maxKey ? `${fmt(val)} / ${fmt(max)}` : undefined}
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, gap: 0 }}>
            {/* Icon + current amount */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <img src={icon} alt={key} style={{ width: 13, height: 13, objectFit: 'contain' }} />
              <span style={{ fontSize: 11, fontWeight: 700, color: valColor, lineHeight: 1 }}>{fmt(val)}</span>
            </div>
            {/* Production rate */}
            {maxKey && rate !== 0 && (
              <span style={{ fontSize: 8.5, color: rate > 0 ? '#5a8' : '#c55', lineHeight: 1, marginTop: 1 }}>
                {rate > 0 ? '+' : ''}{fmt(rate)}/h
              </span>
            )}
            {/* Fill bar */}
            {maxKey && (
              <div style={{ width: '85%', height: 2, borderRadius: 1, background: 'rgba(255,255,255,0.07)', overflow: 'hidden', marginTop: 2 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, color: '#27ae60' }}>${((kingdom as any).usdtBalance || 0).toFixed(3)}</span>
        </div>
      </div>
