...
import { upgradeCost, RESOURCE_META, Cost } from '../utils/costs';
...
export default function BuildScreen() {
  ...
  function mapBuildError(raw: string): string {
  ...
  async function speedUp(type: string) {
    setLoading('speedup-' + type);
    setMsg('');
    try {
      await api.post('/buildings/speedup', { type });
      await refresh();
  ...
  return (
    <div className="screen">
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        ] as const).map(({ key, maxKey, color }) => {
          const val = (kingdom as any)?.[key] ?? 0;
          const max = (kingdom as any)?.[maxKey] ?? 0;
          ...
          return (
            <div key={key} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, fontWeight: 700, color: full ? '#e74c3c' : color }}>
                <span style={{ display:'flex', alignItems:'center', gap:4 }}><ResIcon type={key} size={13} /> {full ? t('storage_full_label') : `${Math.round(pct)}%`}</span>
              </div>
            ...
            <div key={b.id} style={{
              ...
              {b.type === 'town_hall' && (
                <div style={{
                  ...
                  <span style={{ color: '#f4d03f', fontWeight: 700, display:'flex', alignItems:'center', gap:3 }}><ResIcon type="gold" /> {fmt(kingdom?.maxGold ?? 0)}</span>
                  <span style={{ color: '#8B5E3C', fontWeight: 700, display:'flex', alignItems:'center', gap:3 }}><ResIcon type="wood" /> {fmt(kingdom?.maxWood ?? 0)}</span>
                  <span style={{ color: '#aaa', fontWeight: 700, display:'flex', alignItems:'center', gap:3 }}><ResIcon type="stone" /> {fmt(kingdom?.maxStone ?? 0)}</span>
                </div>
              )}
              ...
              {isUpg ? (
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
                    <span>⏳</span>
                    <span>{t('upgrading_label')} <Countdown endsAt={b.upgradeEndsAt} /></span>
                  </div>
                  {(() => {
                    ...
                    return (
                      <button
                        ...
                        style={{ whiteSpace: 'nowrap', fontSize: 12, padding: '6px 10px' }}
                        disabled={loading === 'speedup-' + b.type}
                        onClick={() => speedUp(b.type)}
                      >
                ...
                <>
                  <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
                    {(['gold', 'wood', 'stone'] as const).map(res => {
                      if (cost[res] <= 0) return null;
                  ...
                  <button
                    className="btn btn-gold"
                    style={{ width: '100%', padding: '10px', fontSize: 13, opacity: canAfford ? 1 : 0.55 }}
                    disabled={loading === b.type || !canAfford}
              ...
              {['gold_mine','lumber_mill','stone_quarry','farm'].includes(b.type) && (
